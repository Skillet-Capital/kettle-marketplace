...
const MysteryBoxDebridgeTestModule = buildModule(formatId("MysteryBoxDebridgeTest"), (m) => {
  const { registry } = m.useModule(MysteryBoxRegistryModule);
  const { factory } = m.useModule(KettleAssetFactoryModule);

  const mysteryBox = m.contract("MysteryBoxV1", [
    registry,
    factory,
    ENTROPY_ADDRESS,
...
export default MysteryBoxDebridgeTestModule;
